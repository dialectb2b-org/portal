...
@section('content')
                ...
                <div class="bid-inbox">
                    ...
                    <div class="d-flex w-100 justify-content-between b-bottom">
                        ...
                        <ul class="nav nav-tabs tab mb-2" role="tablist">
                            <li class="nav-item">
                                <a class="nav-link tablinks3" href="{{ route('member.completedBidding.send') }}">Sent</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link tablinks3 active" href="{{ route('member.completedBidding.received') }}">Received</a>
                            </li>
                        </ul>
                        <!-- Tabs navs -->
                    </div>
                    <div class="tabcontent3" id="sent" style="display: block;">
                        <div id="send-list" class="list-group">
                            ...
                            <a href="{{ route('member.completedBidding.received',$enquiry->reference_no) }}" class="list-group-item list-group-item-action flex-column align-items-start enquiry_item">
                                <div class="list-item-inner yellow-border">
                                    ...
                                    <div class="d-flex w-100 justify-content-between">
                                        <small class="bid-date">{{ $enquiry->created_at }}</small>
    ...
    <input type="hidden" id="report-action-url" value="{{ route('member.report') }}" />
     <input type="hidden" id="markuncompleted-action-url" value="{{ route('member.completedBidding.markAsUnCompleted') }}" />
@push('scripts')
...
<script>
    ...
    function openEnquiry(id){
            var fetchEnquiryAction = "{{ route('member.completedBidding.fetchEnquiry') }}";
    ...
    function loadReviewSendList(){
        var fetchSendItemsAction = "{{ route('member.completedBidding.fetchAllReceivedEnquiries') }}";
    ...
    //                                         <ul class="drop">
    //                                             <li  class="markUnCompleted" data-id="${enquiry.id}" data-reference_no="${enquiry.reference_no}">Mark as Uncomplete</li>
    //                                         </ul>
    //                                     </div>
    
            $('body').on('click','.markUnCompleted',function(){
                var id = $(this).data('id');
                var reference_no = $(this).data('reference_no');
                var markUnCompletedAction = $('#markuncompleted-action-url').val();
                Swal.fire({
                    title: "Are you sure?",
                    text: "Do you want to mark #"+reference_no+" as uncompleted!",
                    icon: 'warning',
                    showCancelButton: true,
                }).then(function (willUpdate) {
                    if (willUpdate.isConfirmed === true) {
                        axios.post(markUnCompletedAction, {id:id})
                             .then((response) => {
                                // Handle success response
                                Swal.fire({
                                    toast: true,
