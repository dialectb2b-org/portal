...
@section('content')
    ...
    <!-- Faq answer model starts -->
    @include('procurement.reviewlist.received.suggestion-popup')
    <!-- Faq answer model ends -->
...
<script>
    $( function() {        
                            ...
                            $('.bid-open').append(`<div class="bid-detail-head">
                                ...
                                <div class="d-flex mt-3 justify-content-between">
                                    ...
                                    <div class="add-hold-btn d-flex">
                                        ${reply.enquiry.is_reviewed !== 1 && reply.status == 1 ? 
        ...
        $('body').on('click','.mark-as-selected',function() {
                var reply_id = $(this).data('reply_id');
                var markSelectedAction = "{{ route('procurement.select') }}";
                Swal.fire({
                    title: "Are you sure?",
            ...
            $('body').on('click','.ignore',function() {
                var reply_id = $(this).data('reply_id');
                var markSelectedAction = "{{ route('procurement.ignore') }}";
                Swal.fire({
                    title: "Are you sure?",
            ...
            $('body').on('click','.unselect',function() {
                var reply_id = $(this).data('reply_id');
                var markUnSelectedAction = "{{ route('procurement.unselect') }}";
                Swal.fire({
                    title: "Are you sure?",
    ...
    function loadReviewSendList(){
        var fetchSendItemsAction = "{{ route('procurement.reviewList.fetchAllReceivedEnquiries') }}";
             ...
             // Handle success response
                if(response.data.status === true){
                    ...
                    enquiries.forEach(function(enquiry) {
                        var content = `<a id="enquiry-${enquiry.id}" href="#" data-id="${enquiry.id}" class="list-group-item list-group-item-action flex-column align-items-start enquiry_item received-bg-color">
    ...
    function openEnquiry(id){
        ...
        function setRibbon(enquiry){
            ...
            var reviewableCount = enquiry.selected_count + enquiry.suggestions_count;
            var proceededCount = enquiry.proceeded ? enquiry.proceeded.length : 0;
            
            $('.more-actions').empty();
            if(enquiry.is_reviewed == 0){ 
                var more_action  = `${enquiry.is_reviewed == 0 ? `<a class="ms-2 btn btn-secondary markReviewCompleted" data-id="${enquiry.id}" data-reference_no="${enquiry.reference_no}" data-reviewableCount="${reviewableCount}">Mark as Reviewed</a>` : `` }`;
        ...
        function setMyQuote(enquiry){
            var content = `<div class="mid-second-sec mid-yellow-bg">
                            <h3 class="text-warning">${enquiry.is_reviewed == 1 ? 'Review Completed' : 'Review In Process' }</h3>
                            <div class="d-flex w-100 justify-content-between">
                                <h2 class="received-from-ico">${enquiry.subject}</h2>
             ...
             enquiry.shortlisted.forEach(function(shortlist) {
                            $('#shortlisted_list').append(`<li class="bid-detail" data-reply_id="${shortlist.id}">
                                    <div class="row all-bid-list d-flex align-items-center justify-content-center">
                                        ...
                                        <div class="col-md-3 d-flex align-items-center justify-content-center"><span
                                                class="status-change status-${shortlist.status_color}">${shortlist.status_text}</span></div>
                                    </div>
                                </li>`);
                        });
        }
             ...
             enquiry.selected.forEach(function(select) {
                            $('#selected_list').append(`<li class="bid-detail" data-reply_id="${select.id}">
                                    <div class="row all-bid-list d-flex align-items-center justify-content-center">
                                        ...
                                        <div class="col-md-3 d-flex align-items-center justify-content-center"><span
                                                class="status-change status-${select.status_color}">${select.status_text}</span></div>
                                    </div>
                                </li>`);
                        });
        }
     ...
     $('body').on('click','.markReviewCompleted',function(){
        ...
        var markReviewCompletedAction = "{{ route('procurement.reviewList.markAsReviewed') }}";
