...
<head>
    ...
    <link href="https://fonts.googleapis.com/css2?family=Roboto:ital,wght@0,100;0,300;0,400;0,500;1,100;1,300;1,400;1,500;1,700;1,900&display=swap" rel="stylesheet">
    <link href="https://fonts.googleapis.com/css2?family=Lato:ital,wght@0,900;1,300;1,400&display=swap"  rel="stylesheet">
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.0.0-beta2/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-BmbxuPwQa2lc/FVzBcNJ7UAyJxM6wuqIj61tLrc4wSX0szH/Ev+nYRRuWlolflfl" crossorigin="anonymous">
    <link rel="stylesheet" href="{{ asset('assets/css/style.css') }}" type="text/css">
    <link rel="stylesheet" href="{{ asset('assets/css/style-admin.css') }}" type="text/css">
    <link rel="stylesheet" href="{{ asset('assets/css/style-second.css') }}" type="text/css">
...
<body>
    ...
    <script>
      ...
      jQuery(document).ready(function($) {
            ...
            var notificationAction = "{{ route('procurement.notification') }}";
            axios.get(notificationAction)
                 .then((response) => {
                        ...
                        response.data.announcements.forEach(function(announcement) {
                            ...
                            $('.announcement-box').append(`<a href="#">
                                        ${announcement.title}
                                        <span class="time">${announcement.message}</span>
                                    </a>`);
                        });
                        $('.announcements-count').text(response.data.announcements.length);
                 })
                 .catch((error) => { 
                ...
                var todoAction = "{{ route('procurement.todos') }}";
               
                axios.get(todoAction)
                 .then((response) => {
                        response.data.todos.forEach(function(todo) {
                            $('.todo-list-holder').append(`<li class="d-flex justify-content-between" >
                                    <div><h3 onclick="window.location.href = '${todo.action_url}'">${todo.enquiry_ref}</h3>
                                        <span class="time">${todo.message}</span>
                                    </div>
                                    <a href="#" class="close-small" data-id="${todo.id}"></a>
                                </li>`);
                        });
                        $('.todo-list-holder-count').text(response.data.todos.length);
                })
                .catch((error) => { 
                        // Handle error response
