        ...
        $( function() {
            ...
            function openEnquiry(id){
                    ...
                    axios.post(fetchEnquiryAction, {id:id})
                         .then((response) => {
                            ...
                            $('.bid-open').empty();
                            $('#notify').empty();
                            var openFaqCount = enquiry.open_faqs ? enquiry.open_faqs.length : 0;
                            var closedFaqCount = enquiry.closed_faqs ? enquiry.closed_faqs.length : 0;
                            var pendingRepliesCount = enquiry.pending_replies ? enquiry.pending_replies.length : 0;
                           
                            $('#open-count').text(openFaqCount);
                            $('#closed-count').text(closedFaqCount);
                            $('#pending-count').text(pendingRepliesCount);
            ...
            function setMyQuote(enquiry){
                var content = `<div class="mid-second-sec">
                            ...
                            $('.attachments').append(`<span class="attachmets-list" data-file="{{ config('setup.application_url') }}${attachment.path}">
            ...
            function setBidNotification(enquiry){
                enquiry.pending_replies.forEach(function(pending) {
                    $('#notify').append(`<li class="bid-detail" data-reply_id="${pending.id}">
                                            New Bid from ${pending.sender_company.name}
