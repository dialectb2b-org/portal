                    ...
                    <section class="reg-content-sec">
                        <div class="signup-fields d-flex justify-content-center">
                            <div class="email-otp">
                                ...
                                <span>We've sent you the verification code on<a href="#" id="email">{{ $user['email'] ?? '' }}</a></span>
                                <form id="otp-form" action="{{ route('team.onboard.checkOtp') }}" method="post" class="digit-group d-flex justify-content-center " data-group-name="digits" data-autosubmit="false" autocomplete="off">
                                    @csrf
...
<input type="hidden" id="email_id" value="{{ $user['email'] }}" />
@push('scripts')
<script>
    ...
    $(document).ready(function() {
        ...
        var resend_url = "{{ route('team.onboard.resendOtp') }}";
        $('#email').text(email);
        ...
        const otpInputs = document.querySelectorAll('.input-key');
        //console.log(otpInputs);

        // Function to check if all OTP digits are entered
        function checkOTPComplete() {
            const isOTPComplete = Array.from(otpInputs).every(input => input.value.trim() !== '');
            if (isOTPComplete) {
                checkOtp();
            }
        }
        ...
        otpInputs.forEach(input => {
         ...
         function checkOtp(){
            ...
            $.ajax({
                ...
                success: function(data) {
                    if(data.status === true){
                        window.location.href = "{{ route('team.onboard.declaration') }}";
                    }
                },
                error: function(xhr, status, error) {
