                ...
                <div class="bid-inbox"> 
                    ...
                    <div id="received-list" class="list-group">
                        @forelse($enquiries as $key => $enquiry)
                            <a href="{{ route('sales.draft', ['id' => Crypt::encryptString($enquiry->id)]) }}" class="list-group-item list-group-item-action flex-column align-items-start">
                                <div class="list-item-inner blue-border">
                                    <h2 class="mb-2 round-bullet">{{ $enquiry->enquiry->reference_no }}</h2>
                                    ...
                                    <div class="d-flex w-100 justify-content-between">
                                        <div>
                                            <small class="bid-date">Posted: {{ \Carbon\Carbon::parse($enquiry->enquiry->created_at)->format('d F, Y') }}</small>
                                            <small class="bid-date">Expiry: {{ \Carbon\Carbon::parse($enquiry->enquiry->expired_at)->format('d F, Y') }}</small>
                                        </div>
                        ...
                        <div class="d-flex justify-content-between">
                            ...
                            <div class="form-group proceed-btn float-right">
                                @if($selected_enquiry->enquiry->reply) 
                                    <a id='send-reply' href='#' class='btn btn-secondary' data-id='{{ $selected_enquiry->enquiry->id }}'>Edit Draft</a>
                                @else    
                                    <a id="send-reply" href="#" class="btn btn-secondary" data-id="{{ $selected_enquiry->enquiry->id }}">Reply</a>
                                @endif    
                                <a id="discard" href="#" type="button" class="btn btn-third" data-id="{{ $selected_enquiry->enquiry->id }}">Discard</a>
                            </div>
                        </div>
                        <article>{!! $selected_enquiry->enquiry->body !!}</article>
 ...
 $('#bid-compose-area').hide();
        ...
        $('body').on('click','#send-reply',function(){
            ...
            $('#bid-compose-area').append(`<form id="bid-compose-form" method="post">
                        ...
                        <div class="bid-detail-content reply-msg-white-bg2 position-relative">
                            <textarea id="body" name="body" class="reply-area">${content}</textarea>
                            <small id="body-invalid-msg2" class="text-danger"></small>
                        </div>
                        <div id="attachment-add-preview" class="d-flex align-items-left">
            ...
            formData.append('body', editorContent);
             ...
             axios.post(sendBidAction, formData)
                    .then((response) => {
                        ...
                        location.href = "{{ route('sales.draft') }}";
                    })
                    .catch((error) => { 
                        // Handle error response
                        if (error.response.status == 422) {
        ...
        // Handle success response
            if(response.data.status === true){
                ...
                attachments.forEach(function(attachment) {
                    $("#attachment-add-preview").append(`<a href="#" class="no-attachmets-list d-flex ">${attachment.org_file_name} 
    ...
    function loadReceivedList(){
            var fetchReceivedItemsAction = "{{ route('sales.draft.fetchAllEnquiries') }}";
        ...
        function openEnquiry(id){
            ...
            axios.post(fetchEnquiryAction, {id:id})
                 .then((response) => {
                    // Handle success response
                    console.log(response.data.enquiry.reply);
                    ...
                    var content = `<div class="bid-detail-head">
                                        ...
                                        <div class="d-flex date-status justify-content-between mt-2">
                                            ...
                                            <div class="d-flex">
                                                ...
                                                ${enquiry.sender.company.status == 2 ?
                                                    `<span class="verified">Verified</span>` :
                                                        `<span class="not-verified">Not Verified</span>` }
                                            </div>
                                        </div>
                                    ...
                                    <div class="bid-detail-content">
                                        <div class="d-flex justify-content-between">
                                            ...
                                            <div class="form-group proceed-btn float-right">
                                                ${enquiry.reply ? 
                                                 `<a id='send-reply' href='#' class='btn btn-secondary' data-id='${enquiry.enquiry_id}'' data-reply_id='${enquiry.reply.id}'' data-body='${enquiry.reply.body}'>Edit Draft</a>` :   
                                                 `<a id="send-reply" href="#" class="btn btn-secondary" data-id="${enquiry.enquiry_id}">Reply</a>` }
                                                 <a id="discard" href="#" type="button" class="btn btn-third" data-id="${enquiry.enquiry_id}" >Discard</a>
                                            </div>
                                        </div>
                                        <article>${enquiry.body}</article>
